...
class ECG_CNN(nn.Module):
    """Adaptive pooling makes the head independent of input length.

    The flattened width is still ``conv_channels * pooled_width`` (16*78 by
    default), so checkpoints trained against the original fixed-length model
...
# --------------------------------------------------------------------------- #
# Lazy singletons - built once per process, never per request.
# --------------------------------------------------------------------------- #
_model: Optional[ECG_CNN] = None
_model_ready: Optional[bool] = None   # None = not attempted, False = no weights
_sos: Optional[np.ndarray] = None


def _get_model() -> Optional[ECG_CNN]:
    global _model, _model_ready
    if _model_ready is not None:
        return _model if _model_ready else None

    if not CFG.weights_path.is_file():
        log.error("No ECG weights at %s - ECG will report '%s'.",
    ...
    try:
        ...
        model.eval()
        for p in model.parameters():          # inference only: drop grad buffers
            p.requires_grad_(False)
        _model, _model_ready = model, True
        log.info("Loaded ECG weights from %s", CFG.weights_path)
    except Exception as exc:                  # noqa: BLE001 - must not crash triage
